...
(define (min-tt-aux counters)
  (if (equal? (length counters) 1)
      (counter-tt (car counters))
      (min (counter-tt (car counters)) (min-tt-aux (cdr counters))))
  )
...
(define (min-tt counters)
  (define min (min-tt-aux counters))
  (if (null? counters)
      0
      (if(equal? (counter-tt (car counters)) min)
...
(define (serve requests C1 C2 C3 C4)
  (define (find-case index)
    (cond
      ((= index 1) C1)
       ((= index 2) C2)
       ((= index 3) C3)
       (else C4)))
    (define (fill-case name n-items)
      (if (> n-items ITEMS)
          (add-to-counter (find-case (car (min-tt (list C2 C3 C4)))) name n-items)
          (add-to-counter (find-case (car (min-tt (list C1 C2 C3 C4)))) name n-items)))
  ...
  (if (null? requests)
      ...
      (match (car requests)
        [(list 'delay index minutes)
         (cond
           ((= 1 index) (serve (cdr requests) (tt+ C1 minutes) C2 C3 C4))
           ((= 2 index) (serve (cdr requests) C1 (tt+ C2 minutes) C3 C4))
           ((= 3 index) (serve (cdr requests) C1 C2 (tt+ C3 minutes) C4))
           (else (serve (cdr requests) C1 C2 C3 (tt+ C4 minutes))))]
        [(list name n-items)
         (cond
           ((= 1 (counter-index (fill-case name n-items))) (serve (cdr requests) (fill-case name n-items) C2 C3 C4))
           ((= 2 (counter-index (fill-case name n-items))) (serve (cdr requests) C1 (fill-case name n-items) C3 C4))
           ((= 3 (counter-index (fill-case name n-items))) (serve (cdr requests) C1 C2 (fill-case name n-items) C4))
           (else (serve (cdr requests) C1 C2 C3 (fill-case name n-items))))])))

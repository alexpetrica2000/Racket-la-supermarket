...
(define (enqueue x q)
  (if (< (stream-length (queue-left q)) (length (cons x (queue-right q))))
      (make-queue (rotate (queue-left q) (cons x (queue-right q)) empty-stream) '() (stream-length (rotate (queue-left q) (cons x (queue-right q)) empty-stream)) 0)
      (make-queue (queue-left q) (cons x (queue-right q)) (stream-length (queue-left q)) (length (cons x (queue-right q))))))
...
(define (dequeue q)
  (if (< (- (stream-length (queue-left q)) 1) (length (queue-right q)))
      (make-queue (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream) '() (stream-length (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream)) 0)
      (make-queue (stream-rest (queue-left q)) (queue-right q) (stream-length (stream-rest (queue-left q))) (length (queue-right q)))))
